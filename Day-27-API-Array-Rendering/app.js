...
function initDevLookup() {
  ...
  if (!searchBtn || !usernameInput || !profileContainer || !reposGrid) return;
  ...
  async function fetchContributor(username) {
    ...
    reposGrid.innerHTML = ""; // clear stale repos from a previous search
    ...
    try {
      ...
      fetchRepositories(username); // chain: repos load after profile succeeds
    } catch (error) {
